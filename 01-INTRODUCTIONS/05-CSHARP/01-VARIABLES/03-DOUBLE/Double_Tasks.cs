/*
📘 Module — Basic Data Types: Double (double)
Practice Tasks

This file contains guided practice tasks for C# double variables.

In this module I will:

- Create double variables.
- Perform calculations with decimal numbers.
- Use arithmetic operators.
- Work with prices, percentages, and measurements.
- Practice formatting decimal values.
- Build realistic examples using doubles.

📘 How to use this file:

- Work through the tasks from Rank 1 to Rank 5.
- Write your solutions directly under each task description.
- Do NOT look at the solutions file until you have honestly attempted the task.

📂 There is a separate file:

- Double_Tasks_Solutions.cs
  Contains one possible solution for each task.
*/


// ===========================================================================
// 🟢 Rank 1 → Beginner
// Very simple tasks to get comfortable with doubles.
// ===========================================================================


// 🔹 Task 1.1
//
// Create a double variable called price.
//
// Store:
//
// 19.99
//
// Print the value.
//
// Write your code below:


// 🔹 Task 1.2
//
// Create a double variable called height.
//
// Store:
//
// 1.80
//
// Print the value.
//
// Write your code below:


// 🔹 Task 1.3
//
// Create a double variable called temperature.
//
// Store:
//
// 32.5
//
// Print:
//
// Temperature: 32.5
//
// Write your code below:


// 🔹 Task 1.4
//
// Create a double variable called weight.
//
// Store:
//
// 75.5
//
// Print the value.
//
// Write your code below:


// ===========================================================================
// 🔵 Rank 2 → Easy
// Basic arithmetic.
// ===========================================================================


// 🔹 Task 2.1
//
// Create:
//
// double item1 = 19.99;
// double item2 = 5.50;
//
// Calculate:
//
// total
//
// Print the result.
//
// Expected:
//
// 25.49
//
// Write your code below:


// 🔹 Task 2.2
//
// Create:
//
// double balance = 100.00;
//
// Subtract:
//
// 25.50
//
// Print the result.
//
// Expected:
//
// 74.50
//
// Write your code below:


// 🔹 Task 2.3
//
// Create:
//
// double price = 12.50;
// double quantity = 4;
//
// Calculate total cost.
//
// Print the result.
//
// Expected:
//
// 50.00
//
// Write your code below:


// 🔹 Task 2.4
//
// Create:
//
// double distance = 100.0;
// double hours = 4.0;
//
// Calculate speed.
//
// Print the result.
//
// Expected:
//
// 25.0
//
// Write your code below:


// ===========================================================================
// 🟡 Rank 3 → Intermediate
// Working with decimal values.
// ===========================================================================


// 🔹 Task 3.1
//
// Create:
//
// double criticalChance = 15.5;
//
// Print:
//
// Critical Chance: 15.5%
//
// Write your code below:


// 🔹 Task 3.2
//
// Create:
//
// double pi = 3.14159265359;
//
// Print pi rounded to:
//
// 2 decimal places
//
// Expected:
//
// 3.14
//
// Write your code below:


// 🔹 Task 3.3
//
// Create:
//
// double taxRate = 0.16;
//
// Print:
//
// Tax Rate: 16%
//
// Write your code below:


// 🔹 Task 3.4
//
// Create:
//
// double playerSpeed = 4.75;
//
// Print the value.
//
// Write your code below:


// ===========================================================================
// 🟠 Rank 4 → Advanced
// Real-world examples.
// ===========================================================================


// 🔹 Task 4.1
//
// Create:
//
// double keyboardPrice = 599.99;
// double mousePrice = 299.99;
//
// Calculate cart total.
//
// Print the result.
//
// Write your code below:


// 🔹 Task 4.2
//
// Create:
//
// double packageWeight = 2.75;
// double costPerKg = 4.50;
//
// Calculate shipping cost.
//
// Print the result.
//
// Write your code below:


// 🔹 Task 4.3
//
// Create:
//
// double heightMeters = 1.82;
// double weightKg = 75.5;
//
// Print both values.
//
// Write your code below:


// 🔹 Task 4.4
//
// Create:
//
// double completionRate = 87.5;
//
// Print:
//
// Completion Rate: 87.5%
//
// Write your code below:


// ===========================================================================
// 🔴 Rank 5 → Professional
// Production-style thinking.
// ===========================================================================


// 🔹 Task 5.1
//
// Create a player profile.
//
// Variables:
//
// Attack Speed
// Critical Chance
// Movement Speed
//
// Print all values using clear formatting.
//
// Write your code below:


// 🔹 Task 5.2
//
// Create a shopping cart.
//
// Variables:
//
// Product Price
// Quantity
//
// Calculate total.
//
// Print:
//
// Total Cost: <value>
//
// Write your code below:


// 🔹 Task 5.3
//
// Create a fuel calculator.
//
// Variables:
//
// Distance = 350.5
// FuelUsed = 28.4
//
// Calculate:
//
// Distance / FuelUsed
//
// Print the result.
//
// Write your code below:


// 🔹 Task 5.4
//
// Create a grade calculator.
//
// Variables:
//
// Exam1 = 89.5
// Exam2 = 92.0
// Exam3 = 84.5
//
// Calculate the average.
//
// Print the result.
//
// Write your code below:


/*
👤 Author

Peyman Miyandashti
🎓 Polytechnic University of Baja California
💻 Information Technology Engineering & Digital Innovation
📍 From IRAN (Mexico)
📅 Year: 2026
*/
