/*
📘 Module — Basic Data Types: String (string)
Practice Tasks

This file contains guided practice tasks for C# string variables.

In this module I will:

- Create and print string variables.
- Combine strings using concatenation.
- Format text using string interpolation.
- Use string length.
- Access characters inside strings.
- Use common string methods.
- Clean and validate text.
- Build simple real-world string examples.

📘 How to use this file:

- Work through the tasks from Rank 1 to Rank 5.
- Write your solutions directly under each task description.
- Do NOT look at the solutions file until you have honestly attempted the task.

📂 There is a separate file:

- String_Tasks_Solutions.cs
  Contains one possible solution for each task.
*/


// ===========================================================================
// 🟢 Rank 1 → Beginner
// Very simple tasks to get comfortable with strings.
// ===========================================================================


// 🔹 Task 1.1
//
// Create a string variable called greeting.
// Store:
//
// Hello, World!
//
// Print it.
//
// Write your code below:


// 🔹 Task 1.2
//
// Create a string variable called firstName.
// Store your first name.
// Print it.
//
// Write your code below:


// 🔹 Task 1.3
//
// Create a string variable called city.
// Store the name of your city.
// Print:
//
// City: Mexicali
//
// Write your code below:


// 🔹 Task 1.4
//
// Create a string variable called message.
// Store:
//
// I am learning C# strings.
//
// Print it.
//
// Write your code below:


// ===========================================================================
// 🔵 Rank 2 → Easy
// Combining strings and using interpolation.
// ===========================================================================


// 🔹 Task 2.1
//
// Create:
//
// string firstName
// string lastName
//
// Combine them into:
//
// string fullName
//
// Print:
//
// Full Name: Peyman Miyandashti
//
// Write your code below:


// 🔹 Task 2.2
//
// Create:
//
// string language = "C#";
//
// Print:
//
// I am learning C#.
//
// Use string interpolation.
//
// Write your code below:


// 🔹 Task 2.3
//
// Create:
//
// string school = "Polytechnic University of Baja California";
//
// Print the length of the string.
//
// Example:
//
// Length: 41
//
// Write your code below:


// 🔹 Task 2.4
//
// Create:
//
// string gameName = "Nova Zone";
//
// Print:
//
// Game Name: Nova Zone
//
// Use string interpolation.
//
// Write your code below:


// ===========================================================================
// 🟡 Rank 3 → Intermediate
// Working with characters and methods.
// ===========================================================================


// 🔹 Task 3.1
//
// Create:
//
// string word = "Programming";
//
// Print:
// - First character
// - Last character
//
// Write your code below:


// 🔹 Task 3.2
//
// Create:
//
// string text = "CSharp";
//
// Print the first 3 characters using Substring().
//
// Expected:
//
// CSha
//
// Write your code below:


// 🔹 Task 3.3
//
// Create:
//
// string sentence = "I like Java";
//
// Replace "Java" with "C#".
//
// Print the new sentence.
//
// Expected:
//
// I like C#
//
// Write your code below:


// 🔹 Task 3.4
//
// Create:
//
// string title = "Learning C#";
//
// Check if it contains:
//
// C#
//
// Print the result.
//
// Expected:
//
// True
//
// Write your code below:


// ===========================================================================
// 🟠 Rank 4 → Advanced
// Cleaning and validating text.
// ===========================================================================


// 🔹 Task 4.1
//
// Create:
//
// string rawName = "   peyman   ";
//
// Clean it using Trim().
// Then convert it to uppercase.
//
// Print the final result.
//
// Expected:
//
// PEYMAN
//
// Write your code below:


// 🔹 Task 4.2
//
// Create:
//
// string email = "peyman@example.com";
//
// Check if it contains "@"
// Check if it contains "."
//
// Print both results.
//
// Write your code below:


// 🔹 Task 4.3
//
// Create:
//
// string fileName = "Notes.cs";
//
// Check if the file ends with ".cs".
//
// Print the result.
//
// Expected:
//
// True
//
// Write your code below:


// 🔹 Task 4.4
//
// Create:
//
// string username = "   NovaPlayer   ";
//
// Clean the spaces using Trim().
// Convert it to lowercase.
//
// Print the final result.
//
// Expected:
//
// novaplayer
//
// Write your code below:


// ===========================================================================
// 🔴 Rank 5 → Professional
// Real-world style string processing.
// ===========================================================================


// 🔹 Task 5.1
//
// Create a clean profile message.
//
// Variables:
//
// string name
// string city
// string career
//
// Print:
//
// Name: Peyman
// City: Mexicali
// Career: Information Technology
//
// Use string interpolation.
//
// Write your code below:


// 🔹 Task 5.2
//
// Create a product message.
//
// Variables:
//
// string productName
// double price
//
// Print:
//
// Product: Keyboard | Price: $599.99
//
// Use string interpolation.
//
// Write your code below:


// 🔹 Task 5.3
//
// Create:
//
// string rawUsername = "   PEYMAN NOVA   ";
//
// Clean it by:
// - Removing spaces at the beginning and end
// - Converting it to lowercase
// - Replacing internal spaces with underscores
//
// Expected:
//
// peyman_nova
//
// Write your code below:


// 🔹 Task 5.4
//
// Create:
//
// string password = "Nova12345";
//
// Check:
// - password length is at least 8
// - password contains "123"
//
// Print both results.
//
// Write your code below:


/*
👤 Author

Peyman Miyandashti
🎓 Polytechnic University of Baja California
💻 Information Technology Engineering & Digital Innovation
📍 From IRAN (Mexico)
📅 Year: 2026
*/
